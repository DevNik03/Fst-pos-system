...
const sessionPopupAdd = document.querySelector("#session-open");
const sessionPopupClose = document.querySelector("#session-close");

function mainPopupEnable() {
  sessionPopup.style.visibility = "visible";
  sessionPopup.style.opacity = 1;

  sessionOverlay.style.visibility = "visible";
  sessionOverlay.style.opacity = 1;

  sessionForm.scrollTop = 0;
}

function mainPopupDisable() {
  sessionPopup.style.visibility = "hidden";
  sessionPopup.style.opacity = 0;

  sessionOverlay.style.visibility = "hidden";
  sessionOverlay.style.opacity = 0;

  sessionForm.reset();
}

sessionPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

sessionPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});
...
restaurantNames.forEach((restaurant) => {
  ...
  const id = restaurant.toLowerCase().replace(/\s+/g, "-"); // Convert spaces to dashes for a valid ID
...
const mainSearchBar = document.querySelector("#session-search");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#session-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#session-table tbody input[type='checkbox']:checked"
  );
...
const sessionSubmit = document.querySelector("#session-submit-button");

sessionSubmit.addEventListener("click", () => {
  // Taking inputs
  const sessionName = document.querySelector("#session-name").value.trim();

  function checkedOutlets() {
    const checkedOutletCount = document.querySelectorAll(
      ".outlet-list input[type='checkbox']:checked"
    );

    if (checkedOutletCount.length === 0) {
      // Fix: Check the length properly
      alert("Select At Least One Outlet");
      return false; // Fix: Stop execution if no checkbox is checked
    }
    return true;
  }

  if (!checkedOutlets()) return; // Stop execution if no outlet is checked
  ...
  let isDuplicate = Array.from(existingValues).some(
    (cell) =>
      cell.textContent.trim().toLowerCase() === sessionName.toLowerCase()
  );
...
function recordCount() {
  const rowLength = document.querySelectorAll("#session-table tbody tr").length;
