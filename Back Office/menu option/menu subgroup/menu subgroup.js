...
const popup = document.querySelector(".menu-subgroup-popup");
...
deleteAllCheckbox.addEventListener("change", function () {
  const allCheckboxes = document.querySelectorAll(
    "#menu-subgroup-table tbody input[type='checkbox']"
  );
...
deletebutton.addEventListener("click", () => {
  tbody = document.querySelector("#menu-subgroup-table tbody");
  tbody.innerHTML = "";
  checkEmptyTable();
});
...
const searchBar = document.querySelector("#menu-subgroup-search");
...
const submitButton = document.querySelector("#menu-subgroup-submit");
submitButton.addEventListener("click", (event) => {
  event.preventDefault();

  const menuSubgroupName = document
    .querySelector("#subgroup-name")
    .value.trim();
  ...
  let newValue = menuSubgroupName.toLowerCase();
  ...
  for (let value of existingValues) {
    if (value.textContent.trim().toLowerCase() === newValue) {
      alert("Menu Subgroup Already Exists!");
      return;
    }
  }
