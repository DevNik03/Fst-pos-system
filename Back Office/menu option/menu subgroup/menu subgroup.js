...
const menuSubgroupOverlay = document.querySelector("#menu-subgroup-overlay");
const menuSubgroupPopup = document.querySelector("#menu-subgroup-popup");
const menuSubgroupForm = document.querySelector("#menu-subgroup-form");
const menuSubgroupPopupAdd = document.querySelector("#menu-subgroup-open");
const menuSubgroupPopupClose = document.querySelector("#menu-subgroup-close");

function mainPopupEnable() {
  menuSubgroupPopup.style.visibility = "visible";
  menuSubgroupPopup.style.opacity = 1;

  menuSubgroupOverlay.style.visibility = "visible";
  menuSubgroupOverlay.style.opacity = 1;

  menuSubgroupForm.scrollTop = 0;
}

function mainPopupDisable() {
  menuSubgroupPopup.style.visibility = "hidden";
  menuSubgroupPopup.style.opacity = 0;

  menuSubgroupOverlay.style.visibility = "hidden";
  menuSubgroupOverlay.style.opacity = 0;

  menuSubgroupForm.reset();
}

menuSubgroupPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

menuSubgroupPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});

menuSubgroupOverlay.addEventListener("click", () => {
  mainPopupDisable();
});

//----------------------------- filterbar-----------------------------

const mainSearchBar = document.querySelector("#menu-subgroup-search");
...
function mainSearch() {
  ...
  const tableRows = document.querySelectorAll("#menu-subgroup-table tbody tr");
...
document.addEventListener("DOMContentLoaded", () => {
  const table = document.querySelector("#menu-subgroup-table");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#menu-subgroup-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#menu-subgroup-table tbody input[type='checkbox']:checked"
  );
...
function noDataDisplay() {
  const rows = document.querySelectorAll("#menu-subgroup-table tbody tr");
...
const menuSubgroupSubmit = document.querySelector(
  "#menu-subgroup-submit-button"
);

menuSubgroupSubmit.addEventListener("click", () => {
  // Taking inputs
  const menuSubGroupName = document
  ...
  const existingValues = document.querySelectorAll(
    "#menu-subgroup-table tbody tr td:nth-child(2)"
  );
  let isDuplicate = false;

  existingValues.forEach((cell) => {
    if (
      cell.textContent.trim().toLowerCase() === menuSubGroupName.toLowerCase()
    ) {
      isDuplicate = true;
    }
  });
  ...
  // Populating the data
  const tableBody = document.querySelector("#menu-subgroup-table tbody");
  const row = document.createElement("tr");
  row.innerHTML = `
    <td><input type="checkbox"/></td>
    <td>${menuSubGroupName}</td>
    <td>
...
document
  .querySelector("#menu-subgroup-table tbody")
  .addEventListener("click", (event) => {
