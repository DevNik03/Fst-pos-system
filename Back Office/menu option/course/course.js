...
const courseOverlay = document.querySelector("#course-overlay");
const coursePopup = document.querySelector("#course-popup");
const courseForm = document.querySelector("#course-form");
const coursePopupAdd = document.querySelector("#course-open");
const coursePopupClose = document.querySelector("#course-close");

function mainPopupEnable() {
  coursePopup.style.visibility = "visible";
  coursePopup.style.opacity = 1;

  courseOverlay.style.visibility = "visible";
  courseOverlay.style.opacity = 1;

  courseForm.scrollTop = 0;
}

function mainPopupDisable() {
  coursePopup.style.visibility = "hidden";
  coursePopup.style.opacity = 0;

  courseOverlay.style.visibility = "hidden";
  courseOverlay.style.opacity = 0;

  courseForm.reset();
}

coursePopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});
...
const mainSearchBar = document.querySelector("#course-search");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#course-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#course-table tbody input[type='checkbox']:checked"
  );
...
const CourseSubmit = document.querySelector("#course-submit-button");
...
CourseSubmit.addEventListener("click", () => {
  ...
  existingValues.forEach((cell) => {
    if (cell.textContent.trim().toLowerCase() === courseName.toLowerCase()) {
      isDuplicate = true;
    }
  });
...
document
  .querySelector("#course-table tbody")
  .addEventListener("click", (event) => {
...
function recordCount() {
  const rowLength = document.querySelectorAll("#course-table tbody tr").length;
