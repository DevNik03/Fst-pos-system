...
const waiterOverlay = document.querySelector("#waiter-overlay");
const waiterPopup = document.querySelector("#waiter-popup");
const waiterForm = document.querySelector("#waiter-form");
const waiterPopupAdd = document.querySelector("#waiter-open");
const waiterPopupClose = document.querySelector("#waiter-close");

function mainPopupEnable() {
  waiterPopup.style.visibility = "visible";
  waiterPopup.style.opacity = 1;

  waiterOverlay.style.visibility = "visible";
  waiterOverlay.style.opacity = 1;

  waiterForm.scrollTop = 0;
}

function mainPopupDisable() {
  waiterPopup.style.visibility = "hidden";
  waiterPopup.style.opacity = 0;

  waiterOverlay.style.visibility = "hidden";
  waiterOverlay.style.opacity = 0;

  waiterForm.reset();
}

waiterPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

waiterPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});

waiterOverlay.addEventListener("click", () => {
  mainPopupDisable();
});

const comissions = ["Comission 1", "Comission 2", "Comission 3"];
const comissionSelect = document.querySelector("#waiter-commission");
comissions.forEach((comission) => {
  const option = document.createElement("option");
  option.value = comission;
  option.textContent = comission;

  comissionSelect.appendChild(option);
});
...
const mainSearchBar = document.querySelector("#waiter-search");
...
// -----------------------------table sorting-----------------------------
document.addEventListener("DOMContentLoaded", () => {
  const table = document.querySelector("#waiter-table");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#waiter-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#waiter-table tbody input[type='checkbox']:checked"
  );
...
function noDataDisplay() {
  const rows = document.querySelectorAll("#waiter-table tbody tr");
...
const waiterSubmit = document.querySelector("#waiter-submit-button");

waiterSubmit.addEventListener("click", () => {
  // Taking inputs
  const waiterName = document.querySelector("#waiter-name").value.trim();
  const waiterRole = document.querySelector("#waiter-role").value.trim();
  const waiterOutlet = document.querySelector("#waiter-outlet").value.trim();
  const waiterCommission = document
  ...
  if (waiterRole === "" || waiterRole === "default") {
    alert("Please Select a Role");
    return;
  }

  if (waiterOutlet === "" || waiterOutlet === "default") {
    alert("Please Select an Outlet");
    return;
  }

  let commission;
  if (waiterCommission === "" || waiterCommission === "default") {
    commission = "";
  } else {
    commission = waiterCommission;
  }
  ...
  existingValues.forEach((cell) => {
    if (cell.textContent.trim().toLowerCase() === waiterName.toLowerCase()) {
      isDuplicate = true;
    }
  });
...
function recordCount() {
  const rowLength = document.querySelectorAll("#waiter-table tbody tr").length;
