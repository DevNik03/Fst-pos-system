...
const categoryPopupAdd = document.querySelector("#category-open");
const categoryPopupClose = document.querySelector("#category-close");

function mainPopupEnable() {
  categoryPopup.style.visibility = "visible";
  categoryPopup.style.opacity = 1;

  categoryOverlay.style.visibility = "visible";
  categoryOverlay.style.opacity = 1;

  categoryForm.scrollTop = 0;
}
...
function mainPopupDisable() {
  ...
  categoryOverlay.style.visibility = "hidden";
  categoryOverlay.style.opacity = 0;

  categoryForm.reset();
}

categoryPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

categoryPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});
...
const mainSearchBar = document.querySelector("#category-search");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#category-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#category-table tbody input[type='checkbox']:checked"
  );
...
const categorySubmit = document.querySelector("#category-submit-button");
...
categorySubmit.addEventListener("click", () => {
  ...
  const existingCategories = document.querySelectorAll(
    "#category-table tbody tr td:nth-child(2)"
  );
  let isDuplicate = false;

  existingCategories.forEach((cell) => {
    if (cell.textContent.trim().toLowerCase() === categoryName.toLowerCase()) {
      isDuplicate = true;
    }
  });
