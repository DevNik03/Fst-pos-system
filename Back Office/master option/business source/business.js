...
const form = document.querySelector("#business-source-form");
...
$(document).ready(function () {
  $("#business-source-table").DataTable({
...
deleteAllCheckbox.addEventListener("change", function () {
  const allCheckboxes = document.querySelectorAll(
    "#business-source-table tbody input[type='checkbox']"
  );
...
deletebutton.addEventListener("click", () => {
  tbody = document.querySelector("#business-source-table tbody");
  tbody.innerHTML = "";
  checkEmptyTable();
});
...
const searchBar = document.querySelector("#bs-search");
...
const submitButton = document.querySelector("#bs-submit");

submitButton.addEventListener("click", (event) => {
  event.preventDefault();

  const name = document.querySelector("#bs-name").value.trim();
  const guestReg = document.querySelector("#guest-reg").value.trim();
  const commissionPlan = document.querySelector("#Comission").value.trim();

  // Validation Checks
  if (name === "") {
    alert("Please Enter Business Source");
    return;
  }
  ...
  if (commissionPlan === "" || commissionPlan === "default") {
    alert("Please Select a Commission Plan");
    return;
  }
  ...
  for (let cell of existingAccounts) {
    if (cell.textContent.trim().toLowerCase() === newValue) {
      alert("Duplicate Business Source is not allowed.");
      return;
    }
  }
