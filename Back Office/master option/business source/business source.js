...
const businessSourceOverlay = document.querySelector(
  "#business-source-overlay"
);
const businessSourcePopup = document.querySelector("#business-source-popup");
const businessSourceForm = document.querySelector("#business-source-form");
const businessSourcePopupAdd = document.querySelector("#business-source-open");
const businessSourcePopupClose = document.querySelector(
  "#business-source-close"
);

function mainPopupEnable() {
  businessSourcePopup.style.visibility = "visible";
  businessSourcePopup.style.opacity = 1;

  businessSourceOverlay.style.visibility = "visible";
  businessSourceOverlay.style.opacity = 1;

  businessSourceForm.scrollTop = 0;
}

function mainPopupDisable() {
  businessSourcePopup.style.visibility = "hidden";
  businessSourcePopup.style.opacity = 0;

  businessSourceOverlay.style.visibility = "hidden";
  businessSourceOverlay.style.opacity = 0;

  businessSourceForm.reset();
}

businessSourcePopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

businessSourcePopupClose.addEventListener("click", () => {
  mainPopupDisable();
});

businessSourceOverlay.addEventListener("click", () => {
  mainPopupDisable();
});

// populating commission select
const commissions = [
  "Commission 1",
  "Commission 2",
  "Commission 3",
...
const commissionSelect = document.querySelector("#business-comission");
...
const mainSearchBar = document.querySelector("#business-source-search");
...
function mainSearch() {
  ...
  const tableRows = document.querySelectorAll(
    "#business-source-table tbody tr"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  const table = document.querySelector("#business-source-table");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#business-source-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#business-source-table tbody input[type='checkbox']:checked"
  );
...
function noDataDisplay() {
  const rows = document.querySelectorAll("#business-source-table tbody tr");
...
const businessSourceSubmit = document.querySelector(
  "#business-source-submit-button"
);

businessSourceSubmit.addEventListener("click", () => {
  // Taking inputs
  const businessSourceName = document
    .querySelector("#source-name")
    .value.trim();

  const guestRegistration = document
    .querySelector("#guest-registration")
    .value.trim();

  const comissionPlan = document
    .querySelector("#business-comission")
    .value.trim();
  ...
  if (guestRegistration === "") {
    alert("Please Enter Guest Registration#");
    return;
  }

  let comission;
  if (comissionPlan === "" || comissionPlan === "default") {
    comission = "";
  } else {
  ...
  const existingValues = document.querySelectorAll(
    "#business-source-table tbody tr td:nth-child(2)"
  );

  let isDuplicate = Array.from(existingValues).some(
    (cell) =>
      cell.textContent.trim().toLowerCase() === businessSourceName.toLowerCase()
  );

  if (isDuplicate) {
    alert("Business Source already exists!");
    return;
  }

  // Populating the data
  const tableBody = document.querySelector("#business-source-table tbody");
  const row = document.createElement("tr");
  row.innerHTML = `
    <td><input type="checkbox"/></td>
    <td>${businessSourceName}</td>
...
document
  .querySelector("#business-source-table tbody")
  .addEventListener("click", (event) => {
