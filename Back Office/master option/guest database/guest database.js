...
const guestDBOverlay = document.querySelector("#guest-database-overlay");
const guestDBPopup = document.querySelector("#guest-database-popup");
const guestDBForm = document.querySelector("#guest-database-form");
const guestDBPopupAdd = document.querySelector("#guest-database-open");
const guestDBPopupClose = document.querySelector("#guest-database-close");

function mainPopupEnable() {
  guestDBPopup.style.visibility = "visible";
  guestDBPopup.style.opacity = 1;

  guestDBOverlay.style.visibility = "visible";
  guestDBOverlay.style.opacity = 1;

  guestDBForm.scrollTop = 0;
}

function mainPopupDisable() {
  guestDBPopup.style.visibility = "hidden";
  guestDBPopup.style.opacity = 0;

  guestDBOverlay.style.visibility = "hidden";
  guestDBOverlay.style.opacity = 0;

  guestDBForm.reset();
}

guestDBPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

guestDBPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});

guestDBOverlay.addEventListener("click", () => {
  mainPopupDisable();
});
...
const countrySelect = document.querySelector("#guest-country");
...
const mainSearchBar = document.querySelector("#guest-database-search");
...
// -----------------------------table sorting-----------------------------
document.addEventListener("DOMContentLoaded", () => {
  const table = document.querySelector("#guest-database-table");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#guest-database-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#guest-database-table tbody input[type='checkbox']:checked"
  );
...
function noDataDisplay() {
  const rows = document.querySelectorAll("#guest-database-table tbody tr");
...
const guestDBSubmit = document.querySelector("#guest-database-submit-button");

guestDBSubmit.addEventListener("click", () => {
  // Taking inputs
  const guestName = document.querySelector("#guest-name").value.trim();

  const guestCity = document.querySelector("#guest-city").value.trim();
  const guestCountry = document.querySelector("#guest-country").value.trim();
  ...
  let country;
  if (guestCountry === "" || guestCountry === "default") {
    country = "";
  } else {
  ...
  const existingValues = document.querySelectorAll(
    "#guest-database-table tbody tr td:nth-child(5)"
  );

  let isDuplicate = Array.from(existingValues).some(
    (cell) => cell.textContent.trim().toLowerCase() === guestEmail.toLowerCase()
  );
