...
const emailProviderOverlay = document.querySelector("#email-providers-overlay");
const emailProviderPopup = document.querySelector("#email-providers-popup");
const emailProviderForm = document.querySelector("#email-providers-form");
const emailProviderPopupAdd = document.querySelector("#email-providers-open");
const emailProviderPopupClose = document.querySelector(
  "#email-providers-close"
);

function mainPopupEnable() {
  emailProviderPopup.style.visibility = "visible";
  emailProviderPopup.style.opacity = 1;

  emailProviderOverlay.style.visibility = "visible";
  emailProviderOverlay.style.opacity = 1;

  emailProviderForm.scrollTop = 0;
}

function mainPopupDisable() {
  emailProviderPopup.style.visibility = "hidden";
  emailProviderPopup.style.opacity = 0;

  emailProviderOverlay.style.visibility = "hidden";
  emailProviderOverlay.style.opacity = 0;

  emailProviderForm.reset();
}

emailProviderPopupAdd.addEventListener("click", () => {
  mainPopupEnable();
});

emailProviderPopupClose.addEventListener("click", () => {
  mainPopupDisable();
});

//----------------------------- filterbar-----------------------------

const mainSearchBar = document.querySelector("#email-providers-search");
...
function mainSearch() {
  ...
  const tableRows = document.querySelectorAll(
    "#email-providers-table tbody tr"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  const table = document.querySelector("#email-providers-table");
...
function deleteAllEnable() {
  ...
  const allCheckboxes = document.querySelectorAll(
    "#email-providers-table tr input[type='checkbox']"
  );
...
function deleteRecords() {
  const checkedBoxes = document.querySelectorAll(
    "#email-providers-table tbody input[type='checkbox']:checked"
  );
...
function noDataDisplay() {
  const rows = document.querySelectorAll("#email-providers-table tbody tr");
...
const emailProviderSubmit = document.querySelector(
  "#email-providers-submit-button"
);

emailProviderSubmit.addEventListener("click", () => {
  // Taking inputs
  const emailTitle = document.querySelector("#email-title").value.trim();
  const emailAddress = document.querySelector("#email-address").value.trim();
  const displayName = document
    .querySelector("#email-display-name")
    .value.trim();
  ...
  const existingValues = document.querySelectorAll(
    "#email-providers-table tbody tr td:nth-child(2)"
  );
  ...
  // Populating the data
  const tableBody = document.querySelector("#email-providers-table tbody");
...
// Select the table body where new rows may be added dynamically
const tableBody = document.querySelector("#email-providers-table tbody");

// Function to apply color based on status text
function applyStatusColors() {
  const statusValues = document.querySelectorAll(
    "#email-providers-table tbody tr td:nth-child(5)"
  ); // Re-select dynamically
  statusValues.forEach((value) => {
    const statusText = value.textContent.trim().toLowerCase();
...
document
  .querySelector("#email-providers-table tbody")
  .addEventListener("click", (event) => {
